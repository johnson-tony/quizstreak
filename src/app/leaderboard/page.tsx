    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow max-w-2xl mx-auto px-3 sm:px-4 py-8 md:py-20 w-full space-y-8 md:space-y-12">
        <div className="text-center space-y-3 md:space-y-4">
          ...
          <h1 className="text-3xl md:text-5xl font-black text-foreground tracking-tight leading-tight px-1">World Class Talent</h1>
          <p className="text-sm md:text-base text-muted-foreground font-medium max-w-lg mx-auto px-2">
            The Top 3 engineers globally. Join 10,000+ others and start your journey to the top.
          </p>
        </div>

        <div className="grid gap-3 md:gap-4 max-w-2xl mx-auto px-1">
          {loading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <Skeleton key={i} className="h-20 md:h-24 rounded-2xl md:rounded-3xl" />
            ))
          ) : (
            users.map((user, i) => {
              ...
              return (
                <motion.div 
                  ...
                  key={user._id} 
                  className={`p-4 md:p-6 rounded-2xl md:rounded-[2rem] border-2 transition-all ${i === 0 ? "border-amber-500/20 bg-white shadow-xl shadow-amber-500/5 scale-[1.02] md:scale-105" : "border-primary/5 bg-white/50 shadow-lg"}`}
                >
                  <div className="flex items-center justify-between gap-3 md:gap-4">
                    <div className="flex items-center gap-3 md:gap-6 min-w-0">
                      <div className={`flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center font-black text-base md:text-lg ${style.bg} ${style.color} ${style.border} border shadow-inner`}>
                        {RankIcon ? <RankIcon className="w-5 h-5 md:w-6 md:h-6" /> : i + 1}
                      </div>
                      
                      <div className="flex items-center gap-3 md:gap-4 min-w-0">
                        <Avatar className="h-10 w-10 md:h-14 md:w-14 border-[3px] md:border-4 border-white shadow-xl ring-1 ring-primary/5">
                          <AvatarImage src={user.image} />
                          <AvatarFallback className="bg-primary/5 text-primary font-bold text-sm md:text-base">{user.name[0]}</AvatarFallback>
                        </Avatar>
                        <div className="min-w-0">
                          <h3 className="font-black text-base md:text-xl text-foreground truncate">{user.name}</h3>
                          <div className="flex items-center gap-1.5 mt-0.5 md:mt-1">
                             <div className="text-[9px] md:text-[10px] font-black text-primary/60 uppercase tracking-widest">Elite Member</div>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="text-right flex-shrink-0">
                      <div className="text-lg md:text-2xl font-black text-primary leading-none">{user.totalPoints}</div>
                      <div className="text-[8px] md:text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] mt-1 md:mt-2">Global PTS</div>
                    </div>
                  </div>
                </motion.div>
              );
