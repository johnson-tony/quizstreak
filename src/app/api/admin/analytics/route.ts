...
export async function GET(req: Request) {
  try {
    ...
    if (category !== "all" || type === "weekend_tournament" || type === "custom_duel") {
      poolMatch.poolId = { $in: matchingPools.map((p: any) => p._id) };
    }
    ...
    const [
      ...
      Attempt.aggregate([
        { $match: attemptMatch },
        {
          $group: {
            ...
            cheating: { $sum: { $cond: ["$terminatedDueToCheating", 1, 0] } },
          },
        },
      ]),
      Attempt.aggregate([
      ...
      PoolParticipant.aggregate([
        { $match: poolMatch },
        {
          $group: {
            ...
            revenue: {
              $sum: {
                $cond: [{ $eq: ["$paymentStatus", "verified"] }, "$paidAmount", 0],
              },
            },
          },
        },
      ...
      PoolParticipant.aggregate([
        { $match: { ...poolMatch, paymentStatus: "verified" } },
        {
          $group: {
    ...
    return NextResponse.json({
      ...
      dailyTrend: dailyTrend.map((x: any) => ({
        date: x._id,
        attempts: x.attempts,
        correct: x.correct,
        points: x.points,
      })),
      revenueTrend: poolTrend.map((x: any) => ({
        date: x._id,
        revenue: x.revenue,
        payments: x.payments,
      })),
      topQuestions: topQuestions.map((x: any) => ({
        questionId: x._id,
        attempts: x.attempts,
        correct: x.correct,
        accuracy: Number(x.accuracy.toFixed(1)),
      })),
